...
class Gaym(artstyle.Artyle):
    def __init__(self, master=None, idutc=None):
        super(Gaym, self).__init__(master, idutc=idutc)
        self.disp_img = None
        self.tab_name = "Gaym"
    ...
    def gather_gaym_options(self) -> dict:
        ...
        for ltr in self.gaym_sheet_dict:
            for nbr in self.gaym_sheet_dict[ltr]:
                chosen_gaym_options[ltr].append(self.gaym_sheet_dict[ltr][nbr][0].get())
        chosen_gaym_options["GaymPlay"] = self.spirite_type_str_var.get()
        chosen_gaym_options["MovementProfile"] = random.choice(["LR", "UD"])
        if chosen_gaym_options["GaymPlay"] == "ABF":
    ...
    def create_spirite_sheet(self, img: Image, kre8dict: dict) -> Image:
        ...
        for ltr in ['A', 'B', 'C', 'D']:
            for nbr in range(4):
                fraim = self.stack_layers(kre8dict["Masterpiece"]["Gaym"][ltr][nbr], kre8dict["number_list"],
                ...
                fraim.save(f'GAYM/{kre8dict["artributes"][3]}/{kre8dict["name"]}/{ltr}{nbr}_'
                           f'{kre8dict["Masterpiece"]["Gaym"][ltr][nbr]}.png')
            yh += 1
        return img

    def add_gaym(self, img: Image, kre8dict: dict) -> Image:
        gaym.create_directory(img, kre8dict, gaym_str=kre8dict["Masterpiece"]["Gaym"]["GaymPlay"])
    ...
    def stack_layers(self, spirite_choice: str, nl: list, colist: list, size: (int, int)) -> Image:
        ...
        for color in colist:
            cl.append((int(color[0] * 255),
                       ...
                       int(color[2] * 255),
                       int(1.0 * 255)))
        print(spirite_choice)
        obj_list = gaym.GAYM_OBJECT_DICT[self.spirite_type_str_var.get()]
        lnd = LAYER_DICT[obj_list[spirite_choice][0]]
        layers_list = list(lnd.keys())
        object_image = Image.new('RGBA', (128, 128), (0, 0, 0, 0))
        if True:
            for layer in layers_list:
                pim = Image.open(f'assets/{obj_list[spirite_choice][0]}/{layer}{random.choice(nl)}.png')
